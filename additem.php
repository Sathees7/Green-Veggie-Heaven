...
if (isset($_POST['submit'])) {
    ...
    $result = mysqli_query($con, "SELECT AUTO_INCREMENT FROM information_schema.TABLES WHERE TABLE_SCHEMA = 'GreenVeggieHeaven' AND table_name = 'Items'");
    if (!$result) {
    ...
    if(empty($_POST['itemName']) || empty($_POST['price'] ) || empty($newImageName)){
     ...
     }else{  $sql1 = "INSERT INTO Items (itemName, description, image, catogory, price, quantity) VALUES ('" . $_POST['itemName'] . "','" . $_POST['iDescription'] . "','" . $newImageName . "','" . $_POST['category'] . "'," . $_POST['price'] . "," .$_POST['quentity']  . ")";
...
<div class="container">
  ...
  <input type="text" name="itemName" placeholder="Dish Name" class="box" required>
  <textarea name="iDescription" placeholder="Describe about the dish" class="box"></textarea>
  <input type="file" name="imageFile" placeholder="Upload dish Picture" class="box" required>
  Category  <select name="category" class="box">
    ...
    <input type="text" name="quentity" placeholder="Item quentity" class="box" required> 
