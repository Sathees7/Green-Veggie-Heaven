...
if(isset($id)){
    $resultI = mysqli_query($conn, "SELECT * FROM Promotion WHERE promotionID = $id");

    if ($resultI && mysqli_num_rows($resultI) > 0) {
        $row = mysqli_fetch_assoc($resultI);
        $productName = $row['promotionName'];
...
if (isset($_POST['submit'])) {
    ...
    $result = mysqli_query($conn, "SELECT AUTO_INCREMENT FROM information_schema.TABLES WHERE TABLE_SCHEMA = 'GreenVeggieHeaven' AND table_name = 'Promotion'");
    if (!$result) {
    ...
    if(empty($_POST['promotionName']) || empty($_POST['price'] ) || empty($newImageName)){
        $message[] = 'Please fill out all fields';
    } else {
        $sql2 = "UPDATE Promotion SET promotionName = '" . $_POST['promotionName'] . "', description = '" . $_POST['iDescription'] . "', image = '" . $newImageName . "', price = " . $_POST['price'] . " WHERE promotionID = $id";
        $queryResult = mysqli_query($conn, $sql2);
        if($queryResult){
            $message[] = 'Promotion Updated successfully';
        } else {
            $message[] = 'Could not Update the Promotion';
        }
    }
}
mysqli_close($conn);
...
<div class="container">
   <div class="admin-product-form-container">
      <form action="<?php echo $_SERVER['PHP_SELF'] . '?id=' . $id; ?>" method="POST" enctype="multipart/form-data">
         ...
         <input type="text" name="promotionName" placeholder="Promotion Name" class="box" value="<?php echo isset($productName) ? $productName : ''; ?>" required>
         <textarea name="iDescription" placeholder="Describe About the Promotion" class="box"><?php echo isset($description) ? $description : ''; ?></textarea>
